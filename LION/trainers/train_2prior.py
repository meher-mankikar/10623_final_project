...
from models.dpm_solver_pytorch import DPM_Solver, NoiseScheduleVP, model_wrapper
...
def generate_samples_vada_2prior(shape, dae, diffusion, vae, num_samples, enable_autocast,
    ...
    if ode_sample == 1:
        ...
        if dpm_step > 0:
            scheduler = NoiseScheduleVP(schedule='discrete', alphas_cumprod=diffusion._alpha_bars)
            dae[0].eval()
            dae[1].eval()
        ...
        for i in range(len(dae)):
            if ddim_step > 0:
                assert(cls_emb is None), f'not support yet'
                print("RUNNING DDIM")
                print("LEN DAE", len(dae), ddim_step)
                ...
                x_noisy_size = [num_samples] + shape[i]
                x_noisy = torch.randn(size=tuple(x_noisy_size), device='cuda')

                if i == 0:
                    # global_prior = GlobalPrior(cfg.sde, cfg.latent_pts.style_dim, cfg).cuda()
                    global_fn = model_wrapper(
                        # global_prior,
                        dae[0],
                        scheduler,
                        model_type='noise', # "noise" or "x_start" or "v" or "score"
                    )
                    dpm_solver = DPM_Solver(global_fn, scheduler, algorithm_type="dpmsolver++")
                    eps = dpm_solver.sample(x=x_noisy, steps=dpm_step)
                else:
                    # local_prior = LocalPrior(cfg.sde, cfg.shapelatent.latent_dim, cfg).cuda()
                    model_kwargs = {'condition_input':condition_input}
                    guidance_scale = 1
                    model_fn = model_wrapper(
                        ...
                        unconditional_condition=torch.ones(condition_input.shape, device=condition_input.device),
                        guidance_scale=guidance_scale,
                    )
                    dpm_solver = DPM_Solver(model_fn, scheduler, algorithm_type="dpmsolver++")
                    eps = dpm_solver.sample(
                        x_noisy,
                        steps=dpm_step,
                        order=2,
        ...
        # output['eps_list'] = eps_list TODO: Do we need this?
