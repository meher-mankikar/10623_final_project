...
class Trainer(BaseTrainer):
    ...
    def vis_sample(self, writer, num_vis=None, step=0, include_pred_x0=True,
        ...
        kwargs = {}
        print("In vis sample, ", self.cfg.viz.vis_sample_ddim_step)
        ddim_step = 20
        output = validate_inspect(shape, self.model, self.dae,
                                  ...
                                  ddim_step=200, #self.cfg.viz.vis_sample_ddim_step,
    ...
    def sample(self, num_shapes=2, num_points=2048, device_str='cuda',
        ...
        # ---- forward sampling ---- #
        print("[MEHER] In forward sampling for DDIM in train_prior.py")
        print("THE NUMBER OF SHAPES IS", num_shapes)
        #ddim_step=100
        num_shapes = 1
        ddim_step=40
        t_start = time.time()
        for i in range(10):
            ...
            self.fun_generate_samples_vada(latent_shape, self.dae,
                                           ...
                                           ddim_step=ddim_step,
                                           clip_feat=clip_feat)
            print("Finished sample ", i + 1, nstep, ode_time, sample_time)
        t_end = time.time()
        print(f"Finished generating {num_shapes} samples with {ddim_step} steps. It took {t_end - t_start} seconds")
        return 
        gen_x, nstep, ode_time, sample_time, output_fsample = \
            self.fun_generate_samples_vada(latent_shape, self.dae,
                                           diffusion, self.model, num_shapes,
