...
def main(args, config):
    ...
    if not args.eval_generation:
        ...
        if not args.skip_sample:
            t_start = time.time()
            for i in range(100):
                trainer.vis_sample(num_vis=1, writer=trainer.writer, step=trainer.step, include_pred_x0=False, save_file=None)
                print(f"Done at {i}")
            t_end = time.time()
            print(f"Finished sampling 1000 samples in {t_end - t_start} seconds")
